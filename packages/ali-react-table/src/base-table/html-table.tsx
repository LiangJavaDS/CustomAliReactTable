...
export function HtmlTable({
  ...
  function renderBodyCell(row: any, rowIndex: number, column: ArtColumn, colIndex: number) {
    ...
    if (column.getSpanRect) {
      ...
      // if (isMergeLeafNodes === false) {
      //   // debugger
      //   console.log('5656', row.nodes);
      //   // 行表头的最后一列
      if (row.nodes instanceof Array && row.nodes.length - 1 === colIndex)
        colSpan = 1
      //   // else
      //   if (row.nodes instanceof Array && row.nodes.length - 2 === colIndex) {
      //     debugger
      //     let spanRectTemp = column.getSpanRect(row.nodes[row.nodes.length - 1].value, row, rowIndex)
      //     console.log('7878', row.nodes, colIndex);
      //     colSpan = spanRect == null ? 1 : spanRectTemp.right - colIndex
      //   }
      //   else
    ...
    if (colIndex < leftFlatCount) {
      ...
      positionStyle.left = hozInfo.stickyLeftMap.get(colIndex) || undefined
